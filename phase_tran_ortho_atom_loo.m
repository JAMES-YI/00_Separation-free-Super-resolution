...
%%
succ_prob = [];
DName = sprintf('HMC_phase_tran_orth_loo_data_%s.mat',datestr(now,'yyyymmddHHMM'));
...
%% 
anm = load('ANM_phase_tran_orth_loo_data_201911291858.mat');
hmc = load('HMC_phase_tran_orth_loo_data_201911221108.mat');

figure; hold on;
plot(svec,hmc.succ_prob,'-o');
plot(svec,anm.succ_prob,'-*');
...
print(fig,'Phase_Tran_Orth_Loo','-dpdf');
close all;
